...
/**
 * @coversDefaultClass \SnootBeest\Tantrum\Route\ConstructorProxy
 */
class ConstructorProxyTest extends RouteTestCase
{
    /**
     * @test
     */
    public function constructSetsParams()
    {
        ...
        $constructorProxy = new ConstructorProxy($mockParameterProxyFactory, $mockReflectionMethod);
        self::assertEquals([$mockParameterProxy], $constructorProxy->getParams());
    }

    /**
     * @test
     */
    public function reflectionMethodCanBeNull()
    {
        $mockParameterProxyFactory = $this->mock(ParameterProxyFactory::class);
        $mockParameterProxyFactory->shouldNotReceive('create');
        $constructorProxy = new ConstructorProxy($mockParameterProxyFactory, null);
        self::assertEquals([], $constructorProxy->getParams());
    }
    ...
    public function serializationSucceeds()
    {
        ...
        $constructorProxy = new ConstructorProxy(new ParameterProxyFactory(), $reflectionClass->getConstructor());
        $serialized = serialize($constructorProxy);
        $unserialized = unserialize($serialized);
        $this->assertConstructor($unserialized);
    }
}